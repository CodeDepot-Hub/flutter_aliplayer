...
public class FlutterAliPlayer extends FlutterPlayerBase {
    ...
    public FlutterAliPlayer(FlutterPlugin.FlutterPluginBinding flutterPluginBinding, String playerId) {
        ...
        this.mFlutterInvokeCallback = new FlutterInvokeCallback(mContext, flutterPluginBinding);
        ...
        switch (methodCall.method) {
            ...
            case "setVidSts":
                ...
                if (stsMap.containsKey("playConfig")) {
                    ...
                    Map<String, Object> map = loadGson().fromJson(stsPlayConfig, Map.class);
                    if (null != map) {
                        VidPlayerConfigGen vidPlayerConfigGen = new VidPlayerConfigGen();
                        if (null != map.keySet() && map.keySet().size() > 0) {
            ...
            case "setVidAuth":
                ...
                if (authMap.containsKey("playConfig")) {
                    ...
                    Map<String, Object> map = loadGson().fromJson(authPlayConfig, Map.class);
                    if (null != map) {
                        VidPlayerConfigGen vidPlayerConfigGen = new VidPlayerConfigGen();
                        if (null != map.keySet() && map.keySet().size() > 0) {
                            for (String s : map.keySet()) {
            ...
            case "updateLiveStsInfo":
                ...
                result.success(null);
                break;
            case "switchStream"://切换直播流清晰度
                String newUrl = methodCall.argument("arg");
                mAliPlayer.switchStream(newUrl);
                result.success(null);
                break;
            case "getVideoWidth":
                result.success(mAliPlayer.getVideoWidth());
                break;
            case "getVideoHeight":
                result.success(mAliPlayer.getVideoHeight());
                break;
            case "getVideoRotation":
                result.success(mAliPlayer.getVideoRotation());
                break;
            case "prepare":
                prepare(mAliPlayer);
                result.success(null);
            ...
            case "setSpeed":
                if (mAliPlayer != null) {
                    double value = (Double) methodCall.argument("arg");
                    mAliPlayer.setSpeed((float) value);
                }
                result.success(null);
                break;
            case "getRate":
            ...
            case "setConfig": {
                ...
                if (config != null) {
                    String configJson = loadGson().toJson(setConfigMap);
                    config = loadGson().fromJson(configJson, PlayerConfig.class);
            ...
            case "getConfig":
            case "getPlayConfig":
                result.success(getPlayConfig(mAliPlayer));
                break;
            case "getCacheConfig":
                CacheConfig cacheConfig = getCacheConfig();
                String cacheConfigJson = loadGson().toJson(cacheConfig);
                Map<String, Object> cacheConfigMap = loadGson().fromJson(cacheConfigJson, new TypeToken<Map<String, Object>>() {
                }.getType());
                result.success(cacheConfigMap);
                break;
            case "setCacheConfig":
                Map<String, Object> setCacheConnfigMap = (Map<String, Object>) methodCall.argument("arg");
                String setCacheConfigJson = loadGson().toJson(setCacheConnfigMap);
                CacheConfig setCacheConfig = loadGson().fromJson(setCacheConfigJson, CacheConfig.class);
            ...
            case "setFilterConfig":
                ...
                List<FlutterAliPlayerFilterConfigBean> flutterAliPlayerFilterConfigBeanList = loadGson().fromJson(setFilterConfigJson, new TypeToken<List<FlutterAliPlayerFilterConfigBean>>() {
                }.getType());
            ...
            case "getUserData":
                ...
                break;
            case "setOnRenderFrameCallback":
                FlutterAliPlayerUtils.registerRenderFrameCallback(mAliPlayer, mFlutterInvokeCallback);
                result.success(null);
                break;
            case "setOnPreRenderFrameCallback":
                FlutterAliPlayerUtils.registerPreRenderFrameCallback(mAliPlayer, mFlutterInvokeCallback);
                result.success(null);
                break;
            case "setDrmCallback":
                FlutterAliPlayerUtils.registerDrmCallback(mAliPlayer, mFlutterInvokeCallback);
                result.success(null);
                break;
            ...
            case "setConvertURLCallback":
                AliPlayerFactory.setConvertURLCallback(new IPlayer.ConvertURLCallback() {
                    ...
                    public String convertURL(String s1, String s2) {
                        // 在这里调用 Flutter 端的逻辑并等待其返回结果
                        Object result = mFlutterInvokeCallback.invokeTwoParameterFlutterCallback(s1, s2, "convertURL");
                        if (result == null) {
                            return s1;
                        }
                        return (String) result;  // 返回 Flutter 处理后的结果
                    }
                });
                result.success(null); // 注册成功
                break;
            case "setOnStreamSwitchedListener":
                mAliPlayer.setOnStreamSwitchedListener(new IPlayer.OnStreamSwitchedListener() {
                    @Override
                    public void onSwitchedSuccess(String s) {
                        if (null != mFlutterAliPlayerListener) {
                            Map<String, Object> map = new HashMap<>();
                            map.put("method", "StreamSwitch_onSwitchedSuccess");
                            map.put("playerId", mPlayerId);
                            map.put("url", s);
                            mFlutterAliPlayerListener.onStreamSwitchedSuccess(map);
                        }
                    }

                    @Override
                    public void onSwitchedFail(String s, ErrorInfo errorInfo) {
                        if (null != mFlutterAliPlayerListener) {
                            Map<String, Object> map = new HashMap<>();
                            map.put("method", "StreamSwitch_onSwitchedFail");
                            map.put("playerId", mPlayerId);
                            map.put("url", s);
                            map.put("errorCode", errorInfo.getCode().getValue());
                            map.put("errorMsg", errorInfo.getMsg());
                            mFlutterAliPlayerListener.onStreamSwitchedFail(map);
                        }
                    }
                });
                result.success(null);
                break;
            case "getPlayedDuration"://获取实际播放时长
                long playedDuration = mAliPlayer.getPlayedDuration();
                result.success(playedDuration);
                break;
            case "getDuration"://获取视频时长
                long duration = mAliPlayer.getDuration();
                result.success(duration);
                break;
            case "getCurrentPosition"://获取当前视频位置
                long currentPosition = mAliPlayer.getCurrentPosition();
    ...
    private Map<String, Object> getPlayConfig(AliPlayer mAliPlayer) {
        if (null == mAliPlayer) return null;
        PlayerConfig config = getConfig(mAliPlayer);
        String json = loadGson().toJson(config);
        Map<String, Object> configMap = loadGson().fromJson(json, new TypeToken<Map<String, Object>>() {
        }.getType());
    ...
    private TrackInfo getCurrentTrack(AliPlayer mAliPlayer, int currentTrackIndex) {
        if (mAliPlayer != null) {
            switch (currentTrackIndex) {
                case 0:
                    return mAliPlayer.currentTrack(TrackInfo.Type.TYPE_VIDEO);
                case 1:
                    return mAliPlayer.currentTrack(TrackInfo.Type.TYPE_AUDIO);
                case 2:
                    return mAliPlayer.currentTrack(TrackInfo.Type.TYPE_SUBTITLE);
                case 3:
                    return mAliPlayer.currentTrack(TrackInfo.Type.TYPE_VOD);
            }
        }
        return null;
    }
    ...
    private PlayerConfig mapCovertToPlayerConfig(Map<String, Object> map, PlayerConfig config) {
        if (map != null && config != null) {
            ...
            if (map.containsKey("mSelectTrackBufferMode") && map.get("mSelectTrackBufferMode") != null) {
                config.mSelectTrackBufferMode = (int) map.get("mSelectTrackBufferMode");
            }
        }
        return config;
    }
