...
#define ChannelConvertURL       @"convertURL"
...
- (instancetype)initWithMessenger:(NSObject <FlutterBinaryMessenger> *)messenger {
    ...
    if (self) {
        ...
        FlutterBasicMessageChannel *convertMethodChannel = [[FlutterBasicMessageChannel alloc]initWithName:ChannelConvertURL binaryMessenger:self.binaryMessenger codec:[FlutterStringCodec sharedInstance]];
        [[AliChannelPool sharedManager] addChannel:convertMethodChannel forKey:ChannelConvertURL];
    }
    return self;
}
...
- (void)getPlayedDuration:(NSArray *)arr {
    FlutterResult result = arr[1];
    AliPlayerProxy *proxy = arr[2];
    result(@(proxy.player.getPlayedDuration));
}
...
- (void)setLogOption:(NSArray *)arr {
    FlutterResult result = arr[1];
    FlutterMethodCall *call = arr.firstObject;
    NSNumber *val = [call arguments];
    [AliPlayer setLogOption:FRAME_LEVEL_LOGGING_ENABLED value:val.intValue];
    result(nil);
}
...
//                @"disableVideo": @"mDisableVideo",
                @"enableStrictAuthMode": @"mEnableStrictAuthMode",
                @"selectTrackBufferMode": @"mSelectTrackBufferMode",
                @"positionTimerIntervalMs":@"mPositionTimerIntervalMs",
                @"mMAXBackwardDuration":@"mMaxBackwardBufferDurationMs",
        };
    }];
...
/**
 @brief 切换清晰度（直播流）
 */
- (void)switchStream:(NSArray *)arr {
    FlutterResult result = arr[1];
    AliPlayerProxy *proxy = arr[2];
    NSString *url = arr[3];
    [proxy.player switchStream:url];
    result(nil);
}
...
/**
 @brief 切换清晰度监听设置
 */
- (void)setOnStreamSwitchedListener:(NSArray *)arr {
    FlutterResult result = arr[1];
    AliPlayerProxy *proxy = arr[2];
    
    proxy.player.delegate = proxy;
    
    result(nil);
}

- (void)setOnRenderFrameCallback:(NSArray *)arr {
    FlutterResult result = arr[1];
    AliPlayerProxy *proxy = arr[2];
    proxy.player.renderingDelegate = self;
    result(nil);
}
 
 
- (BOOL)onRenderingFrame:(CicadaFrameInfo *)frameInfo {
    self.renderMethodChannel = [[FlutterBasicMessageChannel alloc]initWithName:@"onRenderFrame" binaryMessenger:self.binaryMessenger codec:[FlutterStringCodec sharedInstance]];
    dispatch_async(dispatch_get_main_queue(), ^{
        if (self.renderMethodChannel){
            [self.renderMethodChannel setMessageHandler:^(id message, FlutterReply reply) {
                reply(nil);
            }];
        }
    });
    ...
    void (^workBlock)(void) = ^{
        NSDictionary *dictionary = @{
            ...
            @"key": @(frameInfo.key),
            @"timePosition": @(frameInfo.timePosition),
            @"audio_format": @(frameInfo.audio_format),
            @"audio_nb_samples": @(frameInfo.audio_nb_samples),
            @"audio_channels": @(frameInfo.audio_channels),
            @"audio_sample_rate": @(frameInfo.audio_sample_rate),
            @"audio_channel_layout": @(frameInfo.audio_channel_layout),
            @"audio_data_lineNum": @(frameInfo.audio_data_lineNum),
            @"audio_data_lineSize": @(frameInfo.audio_data_lineSize),
            @"video_format": @(frameInfo.video_format),
            @"video_width": @(frameInfo.video_width),
            @"video_height": @(frameInfo.video_height),
            @"video_rotate": @(frameInfo.video_rotate),
            @"video_dar": @(frameInfo.video_dar),
            @"video_crop_top": @(frameInfo.video_crop_top),
            @"video_crop_bottom": @(frameInfo.video_crop_bottom),
            @"video_crop_left": @(frameInfo.video_crop_left),
            @"video_crop_right": @(frameInfo.video_crop_right),
            @"video_colorSpace": @(frameInfo.video_colorSpace),
            @"video_colorRange": @(frameInfo.video_colorRange),
            @"video_data_lineNum": @(frameInfo.video_data_lineNum)
        };
        ...
        if (jsonData) {
            ...
            NSString *argments = [NSString stringWithFormat:@"{\"param1\": %@}",jsonString];
//            NSLog(@"[Flutter Send Data] %@",argments);
            dispatch_async(dispatch_get_main_queue(), ^{
                [self.renderMethodChannel sendMessage:argments reply:^(NSNumber *reply) {
                    result = reply.boolValue;
                    NSLog(@"[Flutter] onRenderingFrame-> %d", reply.boolValue);
...
- (void)setConvertURLCallback:(NSArray *)arr {
    FlutterResult result = arr[1];
    AliPlayerProxy *proxy = arr[2];
    [proxy.player setPlayUrlConvertCallback:playUrlCallBack];
    result(nil);
}


#pragma --mark PlayUrlConvertCallback

bool playUrlCallBack(const char* srcURL, const char* srcFormat, char** destURL){
    if ([NSThread isMainThread]){
        return false;
    } else {
        FlutterBasicMessageChannel* convertMethodChannel = [[AliChannelPool sharedManager] channelForKey:ChannelConvertURL];
        if (convertMethodChannel){
            dispatch_async(dispatch_get_main_queue(), ^{
                if (convertMethodChannel){
                    [convertMethodChannel setMessageHandler:^(id message, FlutterReply reply) {
                        reply(nil);
                    }];
                }
            });
            ...
            void (^workBlock)(void) = ^{
                    NSString *argments = [NSString stringWithFormat:@"{\"param1\": \"%s\",\"param2\":  \"%s\"}",srcURL,srcFormat];
                    NSLog(@"[IOS Send Data] %@",argments);
                    dispatch_async(dispatch_get_main_queue(), ^{
                        [convertMethodChannel sendMessage:argments reply:^(NSString *reply) {
                            if (reply == nil || reply.length == 0) {
                                result = false;
                            } else {
                                result = true;
                                char *newurl = strdup([reply UTF8String]);
    
                                size_t length = strlen(newurl) + 1;
    
                                if (*destURL != NULL) {
                                    free(*destURL);
                                }
    
                                *destURL = (char *)malloc(length);
    
                                strcpy(*destURL, newurl);
                            }
