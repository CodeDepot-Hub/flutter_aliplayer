...
- (void)load:(NSArray*)arr {
    ...
    if ([dic objectForKey:@"bandWidth"] != nil) {
        [self.mediaLoader load:dic[@"url"] duration:[dic[@"duration"] longLongValue] defaultBandWidth:[dic[@"bandWidth"] intValue]];
    } else {
...
- (NSString *)checkIfStringIsNull:(NSString *)inputString {
    // 检查输入的字符串是否为nil或空字符串
